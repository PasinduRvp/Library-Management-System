...
import { FaUserMd, FaCalendarAlt, FaUser, FaPhone, FaEnvelope, FaHome, FaStickyNote } from 'react-icons/fa';
import doctorsList from '../helpers/doctorsList';
import SummaryApi from '../common';
import { toast } from 'react-toastify';

const AddHomeVisitAppointment = ({ onClose, fetchData, showSuccessToast }) => {
  ...
  const handleSubmit = async(e) => {
    ...
    try {
      ...
      if(responseData.success) {
        showSuccessToast(responseData.message || "Home visit appointment created successfully!");
        onClose();
        fetchData();
      } else {
        throw new Error(responseData.message || "Failed to create appointment");
        ...
        <div className="bg-white rounded-xl shadow-xl w-full max-w-4xl max-h-[90vh] flex flex-col">
          ...
          <div className="flex justify-between items-center border-b p-6">
            <h2 className="text-2xl font-bold text-gray-800 flex items-center">
              <FaUserMd className="mr-3 text-green-500" />
